...
# k: incapacitate - make it unable to interact (for debugging)

# s: save as {input}
# a: save as {guessed}
# d: save as not fish and advance
# f: save as fish and advance
# x: execute
# g: guess
...
while True:
	...
	img = np.array(ImageGrab.grab(all_screens=True, bbox=((ImageX*screenScale), (ImageY*screenScale), (ImageX+300)*screenScale, (ImageY+350)*screenScale)))
	...
	img_cropped, outline2 = cropToSquare(~cropToSquare(imgBW)[0])
	...
	if lockDown:
		...
		if probability > 0.98:
			if catagory == "Not":
				mouseClick(button1[0], button1[1])
			elif catagory == "Fish":
				mouseClick(button2[0], button2[1])
				updateFrame = 1
			else:
				print("Click action for catagory", catagory, "not registered")
		else:
		...
		cv2.putText(imgRGB, text=catagory, org=TextPoint, fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=2, color=printColor, thickness=3)
	...
	if checkKeyboard('k'):
		...
		print("incapacitated = " + str(incapacitated))

	elif checkKeyboard('d'):
		saveImage(img_cropped, "Not")
		mouseClick(button1[0], button1[1])
		updateFrame = 1
	elif checkKeyboard('f'):
		saveImage(img_cropped, "Fish")
		mouseClick(button2[0], button2[1])
		updateFrame = 1
		...
		print("screenScale", screenScale, "To adjust this permanentaly, edit the screenScale variable")
	elif checkKeyboard('m'):
		ImageX,ImageY = scale(pyautogui.position(), 1/screenScale)
		print("Img", ImageX,ImageY, "To adjust this permanentaly, edit the ImageX,ImageY variables")
	...
	# printColor = (0,255,0) if probability > 0.98 else (0,200,255) if probability > 0.90 else (0,0,255)
	# outImg = cv2.cvtColor(useImg, cv2.COLOR_GRAY2RGB)
	# cv2.putText(outImg, text=catagory, org=TextPoint, fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=2, color=printColor, thickness=3)
	# cv2.imshow("Image", imgRGB)
